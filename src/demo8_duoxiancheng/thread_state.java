...
public class thread_state extends Thread {
    ...
    * join(long millis)表示加入线程执行多少毫秒后，无论线程结束没，其他线程都可以去抢占这个资源
    *
    *
    * 线程礼让
    * 就是在某一个时间点，让线程停止抢占资源，线程状态从就绪/运行》》》》》阻塞状态
    * 线程甲与乙在交替执行，某个时间点，甲礼让，这个时间点乙有了cpu资源，执行业务逻辑
    * 线程甲致使在特定的时间节点礼让，过了就开始抢
    * 通过yeild
    *
    *
    * 线程中断
